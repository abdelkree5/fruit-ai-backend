...
def load_models():
    global seg_model, class_model, qual_model
    ...
    # 3. Load Quality model
    qual_model = load_resnet_model(QUAL_MODEL_PATH, num_classes=2)
...
def predict_quality(image: Image.Image) -> tuple[str, float]:
    if qual_model is None:
        raise RuntimeError("Quality model is not loaded")
        
    input_tensor = preprocess(image).unsqueeze(0).to(device)
    with torch.no_grad():
        outputs = qual_model(input_tensor)
        ...
        quality = QUALITY_CLASSES[class_idx] if class_idx < len(QUALITY_CLASSES) else "unknown"
        return quality, conf_val
...
def analyze_fruit(image: Image.Image) -> dict:
    ...
    # 4. Pass cropped image to Classification and Quality models
    fruit_name, class_conf = predict_class(cropped_image)
    quality, qual_conf = predict_quality(cropped_image)
